...
Optional for MP4 duration:
    - opencv-python, or
    - ffprobe available in system PATH, or
    - moviepy
...
from textgrid import Entry, read_textgrid
...
def get_mp4_duration_with_cv2(mp4_path):
    """
    Try to get MP4 duration with OpenCV.
    """
    try:
        import cv2
    except Exception as exc:
        raise RuntimeError(f"OpenCV is not available: {exc}")

    cap = cv2.VideoCapture(str(mp4_path))

    if not cap.isOpened():
        cap.release()
        raise RuntimeError(f"OpenCV cannot open video: {mp4_path}")
    ...
    if fps is None or fps <= 0 or frame_count is None or frame_count <= 0:
        raise RuntimeError(f"Invalid FPS/frame count from OpenCV for: {mp4_path}")
...
def get_mp4_duration_with_moviepy(mp4_path):
    """
    Try to get MP4 duration with moviepy.
    """
    try:
        from moviepy.editor import VideoFileClip
    except Exception as exc:
        raise RuntimeError(f"moviepy is not available: {exc}")
    ...
    if duration <= 0:
        raise RuntimeError(f"Invalid duration from moviepy for: {mp4_path}")
...
def get_mp4_duration(mp4_path):
    ...
    for method in (
        get_mp4_duration_with_cv2,
        get_mp4_duration_with_ffprobe,
        get_mp4_duration_with_moviepy,
    ):
        try:
            return method(mp4_path)
        except Exception as exc:
            errors.append(f"{method.__name__}: {exc}")

    raise RuntimeError(
        "Cannot read MP4 duration for file: "
        f"{mp4_path}\n" + "\n".join(errors)
    )
...
def intervals_to_targets(
    ...
    # Use ceiling to match the frame boundary calculation method
    num_frames = int(np.ceil(duration / step))

    labels = np.zeros([num_frames, len(ph_classes)], dtype=int)

    for idx, interval in enumerate(phoneme_intervals):
        start_time, stop_time, phone = interval
        phone_lookup = normalize_phoneme_for_lookup(phone)

        check_phone = ph_table[ph_table["Phoneme_Normalized"] == phone_lookup]

        if len(check_phone) > 0:
            start_frame = int(np.ceil(float(start_time) / step))
            stop_frame = int(np.ceil(float(stop_time) / step))

            start_frame = max(start_frame, 0)
            stop_frame = min(stop_frame, num_frames)

            if stop_frame > start_frame:
                labels[start_frame:stop_frame, :] = (
                    check_phone[ph_classes]
                    .iloc[0]
                    .values
                    .astype(int)
                )
            ...
            if print_missing:
                print(
                    "NOT FOUND",
                    f"{filename_prefix}_{idx}" if filename_prefix else idx,
                    phone,
                    "-> lookup as",
                    phone_lookup,
                )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Batch generate phonemic class CSV files from TextGrid/MP4 pairs. "
            "The script recursively searches each subject folder for TextGrid files, "
            "derives each MP4 path by replacing alignments->videos and "
            "alignment->video, crops TextGrid duration to MP4 duration if needed, "
            "and writes one *_label.csv per item."
        )
    )

    parser.add_argument(
        "--root",
        default="F:\\schoolworks\\FAU\\ss26\\mt\\USC-annot-16",
        help="Path to the dataset root folder."
    )

    parser.add_argument(
        "--table",
        default="F:\\schoolworks\\FAU\\ss26\\mt\\program\\TIMIT_MRI_Get_Phone_Alignment\\Phonemic_Table.xlsx",
        help="Path to the phonemic mapping xlsx file, e.g. Phonemic_Table.xlsx."
    )

    parser.add_argument(
        "--output",
        default="F:\\schoolworks\\FAU\\ss26\\mt\\program\\labels_annot_16",
    ...
    parser.add_argument(
        "--tier",
        default="phones",
        help="TextGrid tier name to read. Default: phones.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional regex for subject folder names. "
            "Default: folders whose names start with 'sub'. "
            "Example: 'sub[0-9]+'"
        ),
    )
    ...
    parser.add_argument(
        "--no_print_missing",
        action="store_true",
        help="Do not print phonemes that are missing in the xlsx mapping table.",
    )

    parser.add_argument(
        "--dry_run",
        action="store_true",
        help="Only show what would be processed; do not write CSV files.",
    )
    ...
    process_dataset(
        root=args.root,
        phonemic_table_path=args.table,
        output_root=args.output,
        fps=args.fps,
        tier_name=args.tier,
        sub_regex=args.sub_regex,
        print_missing=not args.no_print_missing,
