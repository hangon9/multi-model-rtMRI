...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "pads a trailing SIL interval when TextGrid is shorter than MP4, and writes one *_label.csv per item."
        )
    )
    ...
    parser.add_argument(
        "--table",
        default="utils/TIMIT_MRI_Get_Phone_Alignment/Phonemic_Table.xlsx",
        help="Path to the phonemic mapping xlsx file, e.g. Phonemic_Table.xlsx."
    )

    parser.add_argument(
        "--output",
        default="datasets/labels_annot_16",
